...
import { Image, ArrowLeft, ChevronLeft, ChevronRight, X } from 'lucide-react';
...
export default function GalleryPage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      <Header />
      <div className="pt-20 sm:pt-24 pb-6 sm:pb-8 px-2 sm:px-4">
        <div className="container mx-auto max-w-6xl">
          {/* Back Button */}
          <div className="mb-4 sm:mb-6">
            <a
              href="/"
              className="inline-flex items-center gap-2 sm:gap-2.5 bg-gray-800 text-white px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl hover:bg-gray-700 transition-all duration-300 hover:scale-105 active:scale-95 shadow-md hover:shadow-lg text-sm sm:text-base font-semibold"
            >
          ...
          <div className="text-center mb-4 sm:mb-6">
            <div className="inline-flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 bg-gray-800 rounded-full mb-2 sm:mb-3">
              <Image size={20} className="sm:hidden text-white" />
              <Image size={24} className="hidden sm:block text-white" />
            </div>
          ...
          {isLoading ? (
            <div className="text-center py-16 sm:py-24">
              ...
              <p className="mt-6 text-gray-600 text-lg font-medium">{t('gallery.loading')}</p>
            </div>
          ) : photoGroups.length === 0 ? (
            <div className="text-center py-16 sm:py-24">
              <div className="bg-white rounded-2xl shadow-lg p-12 max-w-md mx-auto">
                <Image size={64} className="mx-auto text-gray-300 mb-6" />
                <p className="text-gray-600 text-xl font-semibold mb-2">{t('gallery.noPhotos')}</p>
                <p className="text-gray-500 text-sm">{t('gallery.noPhotosDescription')}</p>
              </div>
            </div>
          ) : (
            <div className="relative max-w-2xl mx-auto px-10 sm:px-14">
              {photoGroups.length > 0 && (
                <>
                  {/* Navigation Buttons - Outside box */}
                  ...
                  <div className="bg-white rounded-xl border border-gray-200 shadow-lg overflow-hidden relative">
                    ...
                    {photoGroups.map((group, index) => {
                      ...
                      return (
                        ...
                        >
                        {/* Title Header - Centered and Larger */}
